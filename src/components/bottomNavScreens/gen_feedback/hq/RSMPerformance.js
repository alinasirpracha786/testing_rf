...
export default class RSMPerformance extends React.Component {
  state = {
  ...
  getRSMPerformance_prepare(value) {
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);

      this.getRSMPerformance(value, myHeaders);
  ...
  getRSMPerformance(value, myHeaders) {
    this.setState({ showSpinner: true });
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr_daily/rsm' + '/' + value;
    //console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_big_stream_dvr_daily/rsm/' +
      this.state.selectedRSMValue;
    ////console.warn(url);
    return fetch(url, {
      method: 'GET',
  ...
  getProductWiseDiscount(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_discount_product_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/';
    ////console.warn(url);
    return fetch(url, {
  ...
  getProductWisePayment(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_payment_product_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/';
    ////console.warn(url);
    return fetch(url, {
  ...
  getProductWiseBase(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_subscriber_base_product_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/';
    ////console.warn(url);
    return fetch(url, {
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getAccountWiseDiscount_compnwise(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_daily/rsm/' +
      this.state.selectedRSMValue +'/'+
    ...
    return fetch(url, {
      ...
      .then(responseJs => {
        ...
        if (responseJson.status == 401) {
          ...
          this.setState({ revenueBigStreamTableHead: responseJson.header });
          this.setState({ revenueBigStreamTableData: responseJson.indicators });
          this.setState({ modalSubTitle: title+'|'+vstatus+'|'+ responseJson.mtd_text });
  ...
  getAccountWiseDiscount(title, myHeaders,visitstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_daily/rsm/' +
      this.state.selectedRSMValue +
  ...
  getAccountWisePayment(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_payment_account_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/' + title.trim() + '/' + is_positive;;
    ////console.warn(url);
    return fetch(url, {
  ...
  getAccountWiseBase(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_subscriber_base_account_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/' + title.trim() + '/' + is_positive;;
    ////console.warn(url);
    return fetch(url, {
...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  const url =
    apiHost +
    '/soa_dvr/get_mtd_subscriber_base_product_wise_dvr_daily/rsm/' +
    this.state.selectedRSMValue +'/'+title;
  //console.warn(url);
  return fetch(url, {
    method: 'GET',
    headers: myHeaders
...
getAccountWiseDiscount_compnwise_sellerinfo(title, myHeaders,vstatus) {
 ...
 // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
  ...
  const url =
    apiHost +
    '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_compwise/rsm/' +
    this.state.selectedRSMValue +'/' + title.trim() + '/' + vstatus +'/0/0/';
  //console.warn(url);
  return fetch(url, {
    method: 'GET',
    headers: myHeaders
    ...
    const processCellData = (data, index, rowData) => {
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
          : (['TotalVisits','OpenVisits','ClosedVisits','SuccessfulVisit','seller_indicator','RM','BDO', 'KAM','CSO'].indexOf(data[0]) >= 0 || data[1] == "discount_indicator" || data[1] == "product_revenue_indicator" || data[1] == "gcr_indicator" || data[1] == "dvr_indicator")
            ...
            <View style={styles.rowSelectInput}>
              ...
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getRSMPerformance_prepare.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
                  style={[styles.selectInput, styles.selectInputSmall]}
                />
