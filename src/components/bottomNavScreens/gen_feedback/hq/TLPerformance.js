...
export default class DVRTLPerformance extends Component {
  ...
  getTLsForRD(value = 0) {
    ...
    const url =
      apiHost + '/soa/return_tls_for_rd/' + value + '/0/mtd/1';
  ...
  getRSMsForRD() {
    ...
    const url = apiHost + '/soa/return_rsms_for_rd/0/mtd/1';
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);
  ...
  getAccountWisePerformanceForProduct(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_revenue_account_wise_daily/teamlead/' +
...
//console.warn("seller name "+vstatus);
  this._get("jwt").then((jwt) => {
    var myHeaders = new Headers();
    myHeaders.append('Content-Type', 'application/json');
    myHeaders.append('Authorization', jwt);
