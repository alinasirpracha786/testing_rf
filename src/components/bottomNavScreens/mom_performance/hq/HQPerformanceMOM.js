...
export default class HQPerformanceMOM extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tableDataBottom: [['']],
      selectedsellername: "",
      regionnames:['Central','North','South'],
      title: "",
      modalSubTitle: "",
      widthArr: ['100%'],
      showSpinner: false,
  ...
  getHQPerformance(myHeaders) {
    ...
    const url =
      ...
      '/' +
      this.state.user.username+'/'+this.state.selectedRegionValue;
    //console.warn(url);
    //console.warn("I am under HQ Performance tab: ");
  ...
  getRevenueBigStreamPerformance_prepare() {
    console.warn("revenue performance");
    setTimeout(() => {
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_all_kpi_big_stream/' +
      this.state.user.role +
      '/' +
  ...
  getAccountWisePerformanceForProduct(title, myHeaders) {
    ...
    return fetch(url, {
      ...
      .then(responseJs => {
        ...
        let responseJson = JSON.parse(responseJs);
        if (responseJson.status == 401) {
          //console.log('-----> HERE IN PROMISE THEN IF 401 TRUE');
          this.tokenInvalidResponse("Invalid Token", "Token is not valid anymore, please login again.");
        }

        else {
          // //console.log('-----> HERE IN PROMISE THEN ELSE SECTION', responseJson);



          this.setState({ revenueBigStreamTableHead: responseJson.header });
          // let finalData = responseJson.indicators.reduce(ele => {
        ...
        setTimeout(() => {
          //console.log('-----> HERE IN PROMISE THEN AFTER IF ELSE SETTIMEOUT');
          this.setState({ showSpinner: false });
          this.setState({ modal_revenueBigStreamBreakupVisible: true });
        }, this.state.user.spinnerTimeOutValue);
        this.setState({ modal_revenueBigStreamBreakupVisible: true });
      ...
      .catch(error => {
        //console.log('-----> HERE IN PROMISE CATCH');
  ...
  render() {
    ...
    const processCellData = (data, index, rowData) => {
      ...
      if (typeof rowData != undefined && rowData != null) {
        ...
        let title = ( data[1] == "mom_indicator" ) ? (rowData[0][0]) : rowData[0];
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
          : (['low_performers', 'NULL'].indexOf(data[0]) >= 0  || data[1] == "mom_indicator" )
    ...
    const cellStyleBottom = (cellIndex,data=null) => (
      //styles.singleHeadBottom
      cellIndex === 0 ? styles.singleHead : (cellIndex === 2 ? styles.singleHeadbottom : (data === "Reconnection" ? styles.singleHeadTTPF : styles.singleHeadSixty))
...
<TableWrapper
  ...
  {state.revenueBigStreamTableDatabottom.map((cellData, cellIndex) => (
    ...
    />
          ...
          <View>
            <Text style={styles.title}>{"Low Perfomers IQ Score < 80%\nConsecutive Last 3 months\n"}</Text>
          </View>
         <View style={styles.rowSelectInput}>
