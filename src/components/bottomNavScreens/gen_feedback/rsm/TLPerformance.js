...
export default class DVRTLPerformance extends Component {
  ...
  getTLsForRD() {
    ...
    const url = apiHost + '/soa/return_tls_for_rd/' + this.state.user.username + '/0/mtd/1';
...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  getProductWisePerformanceForBigStream(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_revenue_product_wise_for_big_stream/teamlead/' +
      this.state.selectedTLValue +
      '/' +
  ...
  getAccountWiseDiscount_compnwise_sellerinfo(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      ...
      this.state.selectedTLValue +'/' + title.trim() + '/' + vstatus +'/0/0/';
    //console.warn(url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  render() {
    ...
    const processCellData = (data, index, rowData) => {
      ...
      if (typeof rowData != undefined && rowData != null) {
        ...
        let title = (data[1] == "product_revenue_indicator" || data[1] == "discount_indicator" || data[1] == "payment_indicator" || data[1] == "subscriber_base_indicator" || data[1] == "dvr_indicator") ? (rowData[0][0]) : rowData[0];
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
          :(['TotalVisits','OpenVisits','ClosedVisits','SuccessfulVisit','seller_indicator','RM','BDO', 'KAM','CSO'].indexOf(data[0]) >= 0 || data[1] == "discount_indicator" || data[1] == "product_revenue_indicator" || data[1] == "gcr_indicator" || data[1] == "dvr_indicator")? touchable(data, index, rowData)                    
          ...
          <ScrollView contentContainerStyle={styles.scrollViewContentContainer}>
            <View style={styles.rowSelectInput}>
              ...
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  value={this.state.selectedTLValue.toString()}
