...
export default class TLPerformance extends Component {
  ...
  getTLDormancy_prepare(value) {
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);

      this.getTLDormancy(value, myHeaders);
  ...
  getTLDormancy(value, myHeaders) {
    this.setState({ showSpinner: true });
    //console.warn(value);
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa/get_dormancy/teamlead' + '/' + value;
    //console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getAccountAndProductWiseDormancy(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_dormancy_account_and_product_wise_for_slab/teamlead/' +
            ...
            <View style={styles.rowSelectInput}>
              ...
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getTLDormancy_prepare.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
                  style={[styles.selectInput, styles.selectInputSmall]}
                />
