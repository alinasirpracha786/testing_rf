...
export default class DVRSellerPerformance extends Component {
  ...
  getSellerPerformance(value, myHeaders) {
    ...
    if (value.localeCompare(this.state.user.username) == 0) {
      url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr_daily/seller' + '/' + value;
    }
    else {
      url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr_daily/seller' + '/' + value + '/' + this.state.user.role + "/" + this.state.user.username;
    }
