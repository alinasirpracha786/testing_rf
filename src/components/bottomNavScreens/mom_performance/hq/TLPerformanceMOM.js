...
export default class TLPerformanceMOM extends Component {
  state = {
  ...
  getTLPerformance_prepare() {
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);

      this.getTLPerformance(myHeaders);
  ...
  getTLPerformance( myHeaders) {
    this.setState({ showSpinner: true });
    //console.warn(value);
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa/get_mom_seller_performance/teamlead' + '/' + this.state.selectedTLValue+"/"+this.state.selectedYearValue;
    console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_all_kpi_big_stream/teamlead/' +
      this.state.selectedTLValue;
    //console.warn(url);
    return fetch(url, {
      method: 'GET',
  ...
  getAccountWiseDiscount(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_discount_account_wise/teamlead/' +
      this.state.selectedTLValue +
      '/0/0/' + title.trim() + '/' + is_positive;
    //console.warn(url);
    return fetch(url, {
            ...
            <View style={styles.rowSelectInput}>
              ...
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.onTLSelect.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
                  style={[styles.selectInput, styles.selectInputSmall]}
                />
