...
export default class RDPerformance extends Component {
  ...
  getRegionDormancy_prepare(value) {
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);

      this.getRegionDormancy(value, myHeaders);
  ...
  getRegionDormancy(value, myHeaders) {
    this.setState({ showSpinner: true });
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa/get_dormancy/rd' + '/' + value;
    //console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getAccountAndProductWiseDormancy_prepare(title) {
    ...
    this._get("jwt", "before called").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);
  ...
  getAccountAndProductWiseDormancy(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_dormancy_account_and_product_wise_for_slab//rd' +
      '/' +
  ...
  _get = async (key, param = null) => {

    try {

      let result = await SecureStore.getItemAsync(key);

      //console.log("GET ITEM: ", result, " with param: ", param);
      ...
      <ImageBackground source={{ uri: this.state.user.punch }} style={{ width: "100%", height: "100%" }} imageStyle={{ height: "200%", width: "100%", resizeMode: "repeat" }}>
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.rowSelectInput}>
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getRegionDormancy_prepare.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
          ...
          <View style={styles.pickerLabel}>
            <Text style={styles.pickerLabelText}>Regions:</Text>
          </View>
          <View style={styles.pickerItself}>
            <Picker
              mode="dialog"
              style={{ height: Platform.OS === 'ios' ? 5 : 50, width: '100%', marginRight: 10 }}
              selectedValue={this.state.selectedRD}
              onValueChange={(itemValue, itemIndex) => {
                this.setState({ selectedRD: itemValue });
                this.getRDPerformance(itemValue);
                // Alert.alert('You have selected', itemValue);
              }}>
              {this.state.rds == null ? (
                <Picker.Item label="Loading Regions" />
              ) : (
