...
export default class RDPerformanceMOM extends Component {
  ...
  getRDPerformance(myHeaders,value = null) {
    ...
    const url =
      apiHost +
      'soa/get_mom_seller_performance/rd' +
    ...
    const processCellData = (data, index, rowData) => {
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
          : (['Revenue', 'Discount', 'Base', 'Payment', '1Line', '3i', 'Mobility', 'IaaS','Net Revenue Non-Aggregator','Net Revenue Aggregator','Non-M2M Revenue',
    'M2M Revenue','Net Accounts','Subscriber Base GSM','Subscriber Base OP','IQ Score','Revenue Qualification','New Revenue Achievement',
            'PRI', 'FBB', 'M2M', 'VPBX',
            'BVMB', 'IoT', 'PTT', 'CBS',
            'Zong Track', 'ZFM', 'GSM',
    ...
    return (
      ...
      <ImageBackground source={{ uri: this.state.user.punch }} style={{ width: "100%", height: "100%" }} imageStyle={{ height: "200%", width: "100%", resizeMode: "repeat" }}>
        ...
        <ScrollView style={styles.container}>
          ...
          <View>
            <Text style={styles.title}>{this.state.user.username}{"\n"}{this.state.subTitle}</Text>
          </View>
