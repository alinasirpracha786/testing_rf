...
export default class DVRRSMPerformance extends Component {
  state = {
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getProductWiseGCR(indicator, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_gcr_product_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/' +
      indicator;
    ////console.warn(url);
  ...
  getAccountWiseGCR(product, indicator, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_gcr_account_wise/rsm/' +
      this.state.selectedRSMValue +
      '/0/0/' +
      indicator +
      '/' +
    ...
    const processCellData = (data, index, rowData) => {
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
           : (['TotalVisits','OpenVisits','ClosedVisits','SuccessfulVisit','seller_indicator','RM','BDO', 'KAM','CSO'].indexOf(data[0]) >= 0 || data[1] == "discount_indicator" || data[1] == "dvr_indicator" || data[1] == "product_revenue_indicator" || data[1] == "gcr_indicator")
