...
export default class RDPerformance extends Component {
  ...
  getRDPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_dvr_daily/' +
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_big_stream_dvr_daily/' +
      this.state.user.role+'/'+this.state.user.username;
    //console.warn(url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getAccountWiseDiscount_compnwise_sellerinfo(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      ...
      this.state.user.username +'/'+title.trim() + '/' + vstatus+
      '/0/0/' ;
  ...
  render() {
    ...
    >
          ...
          <View>
            <Text style={styles.title}>{this.rd_to_region_mapping(this.state.user.username)} Region{"\n"}{this.state.subTitle}</Text>
          </View>
