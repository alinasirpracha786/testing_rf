...
export default class DVRSellerPerformance extends Component {
  ...
  getSellersForRD() {
    ...
    const url =
      apiHost + '/soa/return_sellers_for_rd/' + this.state.user.username + '/0/0/' + this.state.switchRM + '/' + this.state.switchKAM + '/' + this.state.switchBDO + '/0/mtd/1';
  ...
  getSellerPerformance(value, myHeaders) {
    ...
    if (value.localeCompare(this.state.user.username) == 0) {
      url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr/seller' + '/' + value;
    }
    else {
      url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr/seller' + '/' + value + '/' + this.state.user.role + "/" + this.state.user.username;
    }
