...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  const url =
    apiHost +
    '/soa_dvr/get_mtd_subscriber_base_product_wise_dvr_daily/rd/' +
    this.state.selectedRDValue +'/'+title;
  //console.warn(url);
  return fetch(url, {
    method: 'GET',
    headers: myHeaders
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_big_stream_dvr_daily/rd/' +
      this.state.selectedRDValue;
    ////console.warn(url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getProductWisePerformanceForBigStream(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_revenue_product_wise_for_big_stream/rd/' +
      this.state.selectedRDValue +
      '/0/0/' + title.trim();
  ...
  getAccountWisePerformanceForProduct(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_revenue_account_wise/rd/' +
      this.state.selectedRDValue +
    ...
    return fetch(url, {
      ...
      .then(responseJs => {
        ...
        if (responseJson.status == 401) {
          ...
          this.setState({ revenueBigStreamTableHead: responseJson.header === null ? 'No Value' : responseJson.header });
          this.setState({ revenueBigStreamTableData: responseJson.indicators === null ? [] : responseJson.indicators });
          this.setState({ modalSubTitle: title + " | Accounts Rev | MTD | " + responseJson.mtd_text === null ? 'No Value' : responseJson.mtd_text });
          ////console.warn('updated state.tableData: ', this.state.revenueBigStreamTableData);
  ...
  getRDPerformance_prepare(value) {
    this._get("jwt").then((jwt) => {
      ...
      this.getRDPerformance(value, myHeaders);
  ...
  getRDPerformance(value, myHeaders) {
    this.setState({ showSpinner: true });
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr_daily/rd' + '/' + value;
    //console.warn('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
        ...
        if (responseJson.status == 401) {
          ...
          this.state.rds.forEach(rd => {
            if (rd.value == value) {
              ...
              this.setState({ selectedRDLabel: rd.label.split('(')[0] + "Region" });
              this.setState({ selectedRDValue: value });
              ////console.warn('UPDATED STATE NAME: ', this.state.selectedRDLabel);
            }
          });
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getAccountWiseDiscount_compnwise(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_daily/rd/' +
      this.state.selectedRDValue +'/'+
    ...
    return fetch(url, {
      ...
      .then(responseJs => {
        ...
        if (responseJson.status == 401) {
         ...
         // this.setState({sellername: title});
  ...
  getAccountWiseDiscount_prepare(title,visitstatus) {
    ...
    //console.warn(title + " called"+visitstatus);
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);
  ...
  getAccountWiseDiscount(title, myHeaders,visitstatus) {
    //console.warn(title + " called");
    //console.warn(visitstatus + " called");
    this.setState({ showSpinner: true });
    const apiHost = this.state.user.apiHost;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_daily/rd/' +
      this.state.selectedRDValue +'/'+title.trim() + '/' + is_positive+
      '/0/0/' ;
  ...
  //console.warn("seller name is "+vstatus);
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);
  ...
  getAccountWiseDiscount_compnwise_sellerinfo(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_compwise/rd/' +
      this.state.selectedRDValue +'/'+title.trim() + '/' + vstatus+
      '/0/0/' ;
  ...
  render() {
    ...
    const processCellData = (data, index, rowData) => {
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
          : (['TotalVisits','OpenVisits','ClosedVisits','SuccessfulVisit','seller_indicator','1Line','RM','BDO', 'KAM','CSO'].indexOf(data[0]) >= 0 || data[1] == "discount_indicator" || data[1] == "product_revenue_indicator" || data[1] == "gcr_indicator" || data[1] == "dvr_indicator")
    ...
    >
            ...
            <View style={styles.rowSelectInput}>
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getRDPerformance_prepare.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
          ...
          <View>
            <Text style={styles.title}>{this.state.selectedRDLabel}{"\n"}{this.state.subTitle}</Text>
          </View>
