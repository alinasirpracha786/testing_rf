...
export default class DVRTLPerformance extends Component {
  state = {
  ...
  getTLsForRD(value = 0) {
    ...
    const url =
      apiHost + '/soa/return_tls_for_rd/' + value + '/' + this.state.user.username + "/mtd/1";
      ...
      return fetch(url, {
        ...
        .then(responseJs => {
          ...
          if (responseJson.status == 401) {
            ...
            // //console.log(' --> get RSM for RD: ', url);
            // //console.log(' --> get RSM for RD: ', responseJson);
            this.setState({ rsms: responseJson });
            ////console.warn('\n------------- rsms: ', this.state.rsms);
  ...
  getSelectedTLLabel(tls, value) {
    tls.forEach(function (tl) {
      if (tl.value == value) {
        ////console.warn('matched name: ', tl.label);
        return tl.label;
      }
    });
  }

  getTLPerformance_prepare(value) {
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);

      this.getTLPerformance(value, myHeaders);
  ...
  getTLPerformance(value, myHeaders) {
    this.setState({ showSpinner: true });
    ////console.warn(value);
    const apiHost = this.state.user.apiHost;

    const url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr_daily/teamlead' + '/' + value;
    //console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
      ...
      .then(responseJs => {
        ...
        if (responseJson.status == 401) {
          ...
          this.state.tls.forEach(tl => {
            if (tl.value == value) {
              ////console.warn('matched name: ', tl.label);
              this.setState({ selectedTLLabel: tl.label + "'s Team" });
              this.setState({ selectedTLValue: value });
              ////console.warn('UPDATED STATE NAME: ', this.state.selectedTLLabel);
            }
          });
  ...
  getAccountWisePerformanceForProduct(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_revenue_account_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  const url =
    apiHost +
    '/soa_dvr/get_mtd_subscriber_base_product_wise_dvr_daily/teamlead/' +
    this.state.selectedTLValue +'/'+title;
  //console.warn(url);
  return fetch(url, {
    method: 'GET',
    headers: myHeaders
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_big_stream_dvr_daily/teamlead/' +
      this.state.selectedTLValue;
    //console.warn(url);
    return fetch(url, {
      method: 'GET',
...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  getProductWiseDiscount(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_discount_product_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getProductWisePayment(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_payment_product_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getProductWiseBase(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_subscriber_base_product_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getAccountWiseDiscount(title, myHeaders,visitstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_daily/teamlead/' +
      this.state.selectedTLValue +
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getAccountWiseDiscount_compnwise(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_daily/teamlead/' +
      this.state.selectedTLValue +'/'+
  ...
  getAccountWisePayment(title, myHeaders) {
    ...
    ////console.warn("this.state.productsDeltaValArray in v: ", this.state.productsDeltaValArray);
    ////console.warn("title: ", title);
    ...
    const url =
      apiHost +
      '/soa/get_mtd_payment_account_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getAccountWiseBase(title, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_subscriber_base_account_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getProductWiseGCR(indicator, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_gcr_product_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getAccountWiseGCR(product, indicator, myHeaders) {
    ...
    const url =
      apiHost +
      '/soa/get_mtd_gcr_account_wise/teamlead/' +
      this.state.selectedTLValue +
      '/' +
      this.state.user.role +
      '/' +
  ...
  getAccountWiseDiscount_compnwise_sellerinfo(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise_compwise/teamlead/' +
      this.state.selectedTLValue +
      '/' + title.trim() + '/' + vstatus +'/0/0/';
    //console.warn(url);
    return fetch(url, {
  ...
  render() {
    ...
    const processCellData = (data, index, rowData) => {
      ...
      if (typeof rowData != undefined && rowData != null) {
        ...
        let title = (data[1] == "product_revenue_indicator" || data[1] == "discount_indicator" || data[1] == "payment_indicator" || data[1] == "dvr_indicator" || data[1] == "subscriber_base_indicator") ? (rowData[0][0]) : rowData[0];
      ...
      return (
        (data.includes("up_") || data.includes("down_"))
          ? element(data, index)
           : (['TotalVisits','OpenVisits','ClosedVisits','SuccessfulVisit','seller_indicator','RM','BDO', 'KAM','CSO'].indexOf(data[0]) >= 0 || data[1] == "discount_indicator" || data[1] == "product_revenue_indicator" || data[1] == "dvr_indicator" || data[1] == "gcr_indicator")? touchable(data, index, rowData)          
   ...
   <Table style={styles.tableStyleModal} borderStyle={{ borderColor: 'transparent' }}>
            ...
            <View style={styles.rowSelectInput}>
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getTLsForRD.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
              ...
              <View style={styles.smallInputWrapper}>
                ...
                <SelectInput
                  ...
                  onCancelEditing={() => { }}
                  onSubmitEditing={this.getTLPerformance_prepare.bind(this)}
                  submitKeyText="Done"
                  cancelKeyText="Close"
                  style={[styles.selectInput, styles.selectInputSmall]}
                />
            ...
            <View style={styles.pickerItself}>
              <Picker
                ...
                onValueChange={(itemValue, itemIndex) => {
                  ...
                  this.getTLPerformance(itemValue);
          ...
          <View>
            <Text style={styles.title}>{this.state.selectedTLLabel}{"\n"}{this.state.subTitle}</Text>
          </View>
