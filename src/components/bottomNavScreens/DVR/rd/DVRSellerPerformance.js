...
export default class DVRSellerPerformance extends Component {
  ...
  getSellersForRD(value = 0) {
    ...
    const url =
      apiHost + '/soa/return_sellers_for_rd/' + value + '/0/' + this.state.user.username + '/' + this.state.switchRM + '/' + this.state.switchKAM + '/' + this.state.switchBDO + '/0/mtd/1';
  ...
  getTLsForRD() {
    ...
    const url =
      apiHost + '/soa/return_tls_for_rd/0/' + this.state.user.username + "/mtd/1";
    this._get("jwt").then((jwt) => {
      var myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      myHeaders.append('Authorization', jwt);
  ...
  getSellerPerformance(value, myHeaders) {
    ...
    const url = apiHost + '/soa_dvr/get_mtd_all_kpi_dvr/seller' + '/' + value;
    //console.log('url', url);
    return fetch(url, {
      method: 'GET',
      headers: myHeaders
  ...
  getRevenueBigStreamPerformance(myHeaders) {
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_all_kpi_big_stream_dvr/seller/' +
      this.state.selectedSellerValue;
    //console.warn(url);
    return fetch(url, {
      method: 'GET',
...
getRevenueBigStreamPerformance_open(myHeaders,title) {
  ...
  const url =
    apiHost +
    '/soa_dvr/get_mtd_subscriber_base_product_wise_dvr/seller/' +
...
getAccountWiseDiscount_prepare(title,visitstatus) {
  ...
  getAccountWiseDiscount(title, myHeaders,visitstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr/seller/' +
      this.state.selectedSellerValue +
...
//console.warn("seller wise info"+title+""+vstatus);
  ...
  getAccountWiseDiscount_compnwise(title, myHeaders,vstatus) {
   ...
   // let is_positive = (this.state.productsDeltaValArray[title].indexOf("-") >= 0) ? 0 : 1;
    ...
    const url =
      apiHost +
      '/soa_dvr/get_mtd_discount_account_wise_dvr_sellerwise/seller/' +
      this.state.selectedSellerValue +'/'+
